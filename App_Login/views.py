...
from Home.models import UserPeriod
...
def sign_up(request):
    ...
    if request.method =='POST':
        ...
        if form.is_valid():
            form.save()
            registered=True
            lp = form.cleaned_data['lastperioddate']
            pd = form.cleaned_data['periodduration']
            un = form.cleaned_data['username']
            queryset = User.objects.all()
            id = queryset.filter(username=un).values('id')
            id = id.first()['id']
            p = UserPeriod(uid_id=id,lasperioddate=lp,duration=pd)
            p.save()
